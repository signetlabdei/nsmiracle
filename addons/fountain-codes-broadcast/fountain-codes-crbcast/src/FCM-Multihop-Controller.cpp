...
enum 
{
    NOT_SET = 65500, TIER = 5, SINGLE = 0, NET = 11, DISCONNECTED = 100, CONNECTED = 40, NEXT_SIMUL = 200
};
...
void FCMMultihopController::newRound()
{
  global_rounds++;
  getCoverage();
...
{
  //current_block++; //spostato sotto
 
  total_rounds += global_rounds;

  if (status == CONNECTED) {
      max_rounds_compl_block[current_block] = global_rounds;
...
void FCMMultihopController::getCoverage()
{
  double compl_nodes = fcm_complete->size();
  net_coverage[global_rounds] += compl_nodes;
  net_cov_freq[global_rounds]++;
}
...
void FCMMultihopController::printStats()
{
  ...
  map<int,double>::reverse_iterator rit = net_coverage.rend();
  //rit++;
  double max_round = (*rit).first;
  //max_round--;

  //cout << "max round è " << max_round << endl;
  //waitForUser();
  double sum_max_rounds = 0;
  for (mit = max_rounds_compl_block.begin() ; mit != max_rounds_compl_block.end() ; mit++) {
       int block = (*mit).first;
       double round = (*mit).second;
       sum_max_rounds += round;
       //cout << "max completion round = " << (int)round << endl;
       int difference = abs(max_round - round);
       //if (difference == 0) cout << "diff = 0!! , rounds " << round << " block = " << (int)block << endl;
       if (difference > 0) {
          ...
          for ( int i = 1; i <= difference; i++) {
              ...
              net_coverage[index] += all_nodes;
              net_cov_freq[index]++;
          }
          //cout << endl;
       }
  }
  sum_max_rounds /= max_rounds_compl_block.size();
  
  cout << "netwrk mean max round = " << sum_max_rounds << endl;

  for (mit = net_coverage.begin() ; mit != net_coverage.end() ; mit++) {
       int round = (*mit).first;
       double nodes = (*mit).second;
       double mean_nodes = (double) nodes / (double) net_cov_freq[round];
       double mean_percentage = mean_nodes / (double) (fcm_pointers->size());
       cout << "netwrk coverage progress: round = " << round << " mean % = " << mean_percentage << endl;
  }
